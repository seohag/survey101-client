...
서버 측에서 AWS S3에 이미지 파일 데이터를 업로드 하는 도중에 에러가 발생했음에도, 클라이언트 측에선 업로드가 완료된 듯한 UI인 설문 공유 링크 모달창이 뜨는 문제점이 있었습니다.
...
이러한 에러핸들링을 해줌으로써 이미지 업로드를 하는 비동기 작업간에 에러가 난다면 AWS S3 bucket에 업로드 하는 과정에서 에러가 났는지, 서버 측의 다른 부분에서 에러가 났는지 확실히 알 수 없기 때문에 클라이언트로 유의미한 에러를 전달하였고, 정확한 에러핸들링을 할 수 있게끔 하였습니다. 이로 인해 에러조차 알 수 없는 상황을 방지하여 개발자 경험과 유저 경험을 향상 시켰습니다.
...
결국 사람에게 필요한 서비스를 제공하는 것이다. 너무 기술에 치우쳐져서도 안되고, 그렇다고 너무.. ~~ 해서도 안되는 트레이드오프 관계를 잘 이해하며 수용하는 쪽으로 적어보는 것도 괜찮을듯 ..
